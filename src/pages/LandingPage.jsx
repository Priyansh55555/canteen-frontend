...
import { getAllMenuLandingPage } from '../hooks/useMenu';
...
const LandingPage = () => {
  const { data: menu, isLoading } = getAllMenuLandingPage();
  const navigate = useNavigate();

  const RightComponent = (
    <div onClick={() => navigate("/")} className="cursor-pointer ml-auto">
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <BackHeader title="Welcome to Canteen" RightComponent={RightComponent} />

      <div className="p-8 max-w-[1400px] w-full mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Our Menu</h1>
          <p className="text-gray-600">Check out our delicious offerings</p>
          <button
            onClick={() => navigate("/")}
                ...
                <div className="p-4 w-full">
                  <div className="flex items-baseline">
                    ...
                    <div className="ml-auto flex gap-2">
                      <span className="bg-gray-200 text-xs px-2 py-1 capitalize rounded-lg">
                        {item.category}
                      </span>
                    </div>
                  </div>
                  ...
                  >
                    {item.isAvailable ? "Login to Order" : "Not Available"}
                  </button>
                </div>
              </div>
            ))}
